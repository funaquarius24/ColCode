...
struct Packet {
    ...
    int iter_number;        // ism iteration number the sending node is currently in

    pair<int, int> task_edge;   // ism contains info on (dst_task_id, edge_id)
    pair<int, int> packet_sequence; // ism contains info on (number_of_packets_in_edge, number_this_flit_belongs)
    ...
    Packet(const int s, const int d, const int vc, const double ts, const int sz, const pair<int, int> t_e) {
	    make(s, d, vc, ts, sz);
        task_edge = t_e;
    }
...
struct Flit {
	...
	return (flit.src_id == src_id && flit.dst_id == dst_id
        ...
        && flit.task_edge == task_edge
        && flit.packet_sequence == packet_sequence;
}};
...
struct TaskInt
{
    int id;
    int position;
    vector<pair<int, int>> incoming_edge_dep_list; // ism vector<pair<edgeID, position_in_edge_list>>
    vector<pair<int, int>> outgoing_edge_dep_list; // ism vector<pair<edgeID, position_in_edge_list>>
};
