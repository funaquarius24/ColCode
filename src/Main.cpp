...
//assert(false);
    ...
    GlobalStats gs(n);
    gs.showStats(std::cout, GlobalParams::detailed);
    map<int, int> allPacketCounts = gs.aggregatePacketCounts(sc_time_stamp().to_double()); //all packet counts will be aggregated and counted here
    map<int, int>::iterator it;

    FILE *file;
    const char * filename="../outFolder/outfile.csv";
