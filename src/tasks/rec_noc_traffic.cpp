...
extern bool TO_PRINT_REC;
extern int num_line_header;

std::vector<RecProc, std::allocator<RecProc>> RecNOCTraffic::load_traffic(string trace_file) {
	...
	FILE *fp = fopen("../src/traffics/Sparse_mesh_8x8.rtp", "r");
	if (fp == NULL) {
	...
	fscanf(fp, "%d", &num_iter);
	assert (topology>=0 && num_proc>0 && num_row>=0 && num_col>=0);
	assert (num_task>0 && num_edge>0 && num_iter>0);
	...
	for (int i=0; i<num_task; i++) {
		...
		// task id, mapped PB id/coordinate, vector of schedule sequence numbers, vector of recorded task execution times
		fscanf(fp, "%d", &v[0]);
		int proc_id = -1;
		if (topology == 0 || topology == 1) {
			fgetc(fp);
		...
		t.set_schedule(v1);
		t.set_recorded_execution_time(v2);
	...
	for (int i=0; i<num_edge; i++){
		...
		// vector of memory addresses, vector of recorded message sizes
		fscanf(fp, "%d", &v[0]);
		fscanf(fp, "%d", &v[1]);
		fscanf(fp, "%d", &v[2]);
		for (int j=0; j<num_iter; j++) {
		...
		e.set_mem_addr_list(v1);
		e.set_recorded_msg_size(d1);

		assert (v[0] >= 0 && v[0] < (int)edge_list.size());
		edge_list.at(v[0]) = e;

		RecTask &src_task = task_list.at(v[1]);
		RecTask &dst_task = task_list.at(v[2]);
...
int	RecNOCTraffic::print_traffic() {

	cout << "Type: recorded traffic pattern" << endl << endl;
	switch(topology) {
	case 0: 
	...
	cout << "Number of edges:		" << get_num_edge() << endl;
	cout << "Number of iterations:		" << get_num_iter() << endl << endl;
...
RecProc* RecNOCTraffic::get_proc(int x, int y) {
...
RecProc* RecNOCTraffic::get_proc(int id) {
...
int	RecNOCTraffic::get_num_task() {

	return task_list.size();
}

int	RecNOCTraffic::get_num_edge() {

	return edge_list.size();
}

int	RecNOCTraffic::get_num_proc() {

	return proc_list.size();
}

int	RecNOCTraffic::get_topology() {

	return topology;
}

int	RecNOCTraffic::get_num_row() {

	return num_row;
}

int	RecNOCTraffic::get_num_col() {

	return num_col;
}

int	RecNOCTraffic::get_num_iter() {
