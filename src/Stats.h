...
class Stats {
  ...
  public:
    ...
    void fullPacket(const double arrival_time);
    pair<int, int> getNumReceivedPackets(const double arrival_time); //get the number of received packets for a particular time
    map<int, int> getNumberOfPackets(); //get the object numberOfPackets
  ...
  private:
    ...
    double warm_up_time;
    map<int, int> numberOfPackets; //Number of packets using every 100 cycles as keys
