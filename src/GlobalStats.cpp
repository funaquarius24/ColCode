...
map<int, int> GlobalStats::aggregatePacketCounts( const double simTime ){
	int count = simTime / 1000000;

	for(int i = 0; i <= count; i++ ){
		allFullPackets.insert(pair<int, int>(i, 0));
	}

	for (int y = 0; y < GlobalParams::mesh_dim_y; y++)
	    for (int x = 0; x < GlobalParams::mesh_dim_x; x++){
			map<int, int> fullPackets = noc->t[x][y]->r->stats.getNumberOfPackets();
			for(int i = 0; i <= count; i++ ){
				allFullPackets[i] = allFullPackets[i] + fullPackets[i];
			}
		}
	return allFullPackets;
