...
void Stats::configure(const int node_id,
...
void Stats::fullPacket( const double arrival_time )
{
    
    int time_interval = (int)arrival_time / 1000;
    map<int, int>::iterator iter = numberOfPackets.find(time_interval);

    if(iter == numberOfPackets.end())
    {
        numberOfPackets.insert(pair<int, int>(time_interval, 1));
...
map<int, int> Stats::getNumberOfPackets(){
