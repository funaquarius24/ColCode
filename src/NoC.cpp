...
void NoC::buildCommon() {
    ...
    for (map < int, ChannelConfig > ::iterator it = GlobalParams::channel_configuration.begin(); it != GlobalParams::channel_configuration.end();
        ++it) {
    ...
    char hub_name[16];
    for (map < int, HubConfig > ::iterator it = GlobalParams::hub_configuration.begin(); it != GlobalParams::hub_configuration.end();
        ++it) {
        int hub_id = it -> first;
        //LOG << " hub id " <<  hub_id;
        ...
        for (vector < int > ::iterator iit = hub_config.attachedNodes.begin(); iit != hub_config.attachedNodes.end();
            ++iit) {
        ...
        for (vector < int > ::iterator iit = hub_config.txChannels.begin(); iit != hub_config.txChannels.end();
            ++iit) {
            int channel_id = * iit;
        ...
        for (vector < int > ::iterator iit = hub_config.rxChannels.begin(); iit != hub_config.rxChannels.end();
            ++iit) {
            int channel_id = * iit;
...
void NoC::buildMesh() {
    ...
    //just a dummy
    string trace_file;
	rec_noc.load_traffic(trace_file);
    stat_noc.load_traffic(trace_file);  
    //
    //cout << rec_noc.get_proc(3) -> print_all_task(2)  << endl;
    //
    ...
    for (int j = 0; j < GlobalParams::mesh_dim_y; j++) {
        for (int i = 0; i < GlobalParams::mesh_dim_x; i++) {
            ...
            t[i][j] -> pe -> rec_proc = rec_noc.get_proc(t[i][j] -> pe -> local_id);
            t[i][j] -> pe -> stat_proc = stat_noc.get_proc(t[i][j] -> pe -> local_id);
            if(t[i][j] -> pe -> local_id == 0){
                //RecProc rec_proc = *rec_noc.get_proc(t[i][j] -> pe -> local_id);
                //cout << "first task: " << rec_proc.task_list.front() << endl;
            }
            //t[i][j] -> pe -> rec_proc = *rec_noc.get_proc(t[i][j] -> pe -> local_id);
